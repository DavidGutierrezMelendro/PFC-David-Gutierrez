...
Currently the web browsers have became an essential element for the Internet navigatio, the continuos developments on web technologies has delivered numerous tools strongly related to the web browsers.
...
El objetivo principal, es por tanto, elaborar una aplicación web que permita interactuar al potencial cliente de un proveedor de Internet o ISP, con la solución de \textit{CloudTrust}.
Este proyecto pretende dar respuesta a su vez, a otra serie de cuestiones, se enumeran a continuación:
\begin{itemize}
\item   
...
\section{Python 3}
El lenguaje de programación en el que está basado Django es Python\cite{wPython}, y más concretamente en su versión 1.6.5, hace uso de Python 3, lanzado en Diciembre de 2008. 
...
HTML\cite{w3school}, siglas de HyperText Markup Language, se trata de un lenguaje de marcado para la elaboración de páginas web. Es un estándar mundialmente utilizado, y de enorme uso que pretende homogeneizar el código en la web. El W3C (World Wide Web Consortium) está a cargo del estándar. Se trata del estándar que se ha impuesto a la hora de visualizar páginas web y el que todos los navegadores han adoptado.
...
\begin{enumerate}
  \item El primero de estos ficheros, "InitialScript.py" contiene una serie de instrucciones que ejecutan y arrancan las funcionalidades principales del escenario como son el Controlador y el ServidorWeb de Django.

  \item El segundo de los ficheros, "floodlightInitialScript.py" corresponde al arranque de los servicios requeridos por el Controlador Floodlight.
