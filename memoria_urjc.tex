...
Currently web browsers have become an essential element for the Internet navigatio, the continuos developments on web technologies has delivered numerous tools strongly related to the web browsers.
...
\begin{itemize}
\item
Crear un entorno de desarrollo virtual con VirtualBox que simule un escenario real de red doméstica, que no comprometa ni pierda funcionalidades básicas de conectividad de los servicios tradicionales.
\item
...
Hacer uso de varias herramientas de código libre, como pueden ser Django, Python ó HTML durante la fase de desarrollo, implementación y explotación del proyecto.
...
\item
Identificar cuales son los requisitos y especificaciones que requiere un proyecto de esta embergadura. Así cómo, elaborar los puntos de acción y responsabilidades de los agentes que van a contribuir en el proyecto.
\item
...
\item
Elaborar una página web del proyecto (http://\textit{CloudTrust}.grupoinnovati.com/) así como una cuenta de Twitter que dé visibilidad al proyecto (https://twitter.com/mihogarenlanube).
...
\begin{itemize}
  ...
  \item Administrador: Entenderemos por administrador a aquel usuario con el rango de superuser que tendrá determinados privilegios (definidos en la especificación).
\end{itemize}
...
\begin{itemize}
\item
ESP.APPWEB.CONF.001 - Cuando se realice la instalación de la red \textit{CloudTrust} en el hogar por primera vez, se creará un usuario administrador por defecto (username), una contraseña de username administrador, así como una contraseña de acceso a la red.
\item
...
Así, una de las primeras decisiones que se tomaron fue el uso del framework de Django~\cite{wDjango}. Django es un framework de alto nivel basado en Python que promueve la creación de aplicaciones web de forma rápida, limpia y con diseños pragmáticos.
...
Otra de las herramientas principales de las que se ha hecho uso dentro del desarrollo de la aplicación web es Bootstrap~\cite{wBoot}. Bootstrap es un framework de código libre de Twitter limpio, intuitivo y rápido, para el desarrollo de páginas web.
...
En su repositorio GitHub oficial podemos encontrar páginas con toda la información necesaria para utilizar Bootstrap en nuestro proyecto web, además también podemos encontrar ejemplos de utilización de este Framework.
...
\section{Python 3}
El lenguaje de programación en el que está basado Django es Python~\cite{wPython}. Más concretamente la versión 1.6.5 de Django hace uso de Python 3, la nueva versión de Python, lanzada en Diciembre de 2008. 
...
Como mencionamos anteriormente, Django incluye un servidor web liviano para realizar pruebas y trabajar en la etapa de desarrollo. En la etapa de producción, sin embargo, se recomienda  Apache 2~\cite{wApache} con mod\_ python. Aunque Django soporta la especificación WSGI, por lo que puede correr sobre una gran variedad de servidores como FastCGI o SCGI en Apache u otros servidores (particularmente Lighttpd).
...
HTML~\cite{w3school}, siglas de HyperText Markup Language, es un lenguaje de marcado para la elaboración de páginas web. Es un estándar mundialmente utilizado, y de enorme uso que pretende homogeneizar el código en la web. El W3C (World Wide Web Consortium) está a cargo del estándar. Se trata del estándar que se ha impuesto a la hora de visualizar páginas web y el que todos los navegadores han adoptado.
...
Open vSwitch~\cite{wvSwitch}: Switch implementado por software que dispone de compatibilidad para integrarse con diversos protocolos de control y monitorización como Openflow, Netflow, Sflow, etc.

\section{Frenetic}
La arquitectura de control SDN propuesta en el proyecto Frenetic~\cite{wFrenetic} se compone de tres niveles.
\begin{itemize}
\item
POX: Este es el nivel de abstracción más bajo de la arquitectura. Se encarga de actuar como interfaz para el protocolo Openflow en ambas direcciones. Ofrece una API para el empleo del protocolo Openflow en lenguaje Python.
...
Además de todo el potencial que ofrece la solución \textit{CloudTrust}, en lo que concierne al usuario final el proyecto se centrará en la aplicación de configuración web.
...
{\scriptsize
  \begin{verbatim}
vnx -f \textit{CloudTrust}-v03.xml -v -t
vnx -f \textit{CloudTrust}-v03.xml -v -x start-srv
vnx -f \textit{CloudTrust}-v03.xml -x start-parental
vnx -f \textit{CloudTrust}-v03.xml -v -start -M h1
\end{verbatim}
}
...
Como se ha comentado anteriormente, Django requiere de una estructura de contenidos peculiar y precisa. Por lo que con esta imagen se quiere mostrar y explicar el porqué del árbol de carpetas que se presenta en la solución \textit{CloudTrust}.
...
\begin{enumerate}
  ...
  \item El segundo de los ficheros, \texttt{floodlightInitialScript.py}, corresponde al arranque de los servicios requeridos por el Controlador Floodlight.
...
\bibitem{w3school} Web w3school, The largest web developer site, HTML y CSS \\
{\footnotesize
